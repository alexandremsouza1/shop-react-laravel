...
class DataScraping extends Controller
{
    ...
    public function marcas($vehicle_type_id){
        ...
        foreach ($vehicle_brand->values as $brand) {
    ...
    public function carro(){
        ...
        foreach ($tables as $item) {
           $item = (object) $item;
           foreach($item->data->values as $value){
        ...
        foreach($data[11]->values as $features_car){
    ...
    public function moto(){
        ...
        foreach ($data[3]->values as $value) {
            $valid =  Vehicle_cubccms::where('value',$value->value)->first();
            
            if(empty($valid)){
                Vehicle_cubccms::create((array)$value);
            }
        }

        foreach($data[5]->values as $moto_features){
